...
function loadConfig() {
  ...
  const isDebuging = process.execArgv.join().includes('inspect');
  ...
  logger.info(`Host: ${process.env.HOSTNAME}, Process: ${process.pid}, instance: ${instance}`);
  logger.info('Node:', process.version);
  logger.info('NODE_ENV:', process.env.NODE_ENV);  // "production"
  logger.info('isDebugging:', isDebuging);
